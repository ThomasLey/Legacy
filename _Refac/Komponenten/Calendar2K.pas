unit Calendar2K;
{ This component was written by Borland Inc. and changed by
    Thomas Ley - tley@gmx.de
    Date: 10.10.2K
}
...
constructor TCalendar2K.Create(AOwner: TComponent);
begin
...
procedure TCalendar2K.Change;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TCalendar2K.Click;
var
  TheCellText: string;
begin
  inherited Click;
...
function TCalendar2K.IsLeapYear(AYear: Integer): Boolean;
begin
  Result := (AYear mod 4 = 0) and ((AYear mod 100 <> 0) or (AYear mod 400 = 0));
end;

function TCalendar2K.DaysPerMonth(AYear, AMonth: Integer): Integer;
const
...
function TCalendar2K.DaysThisMonth: Integer;
begin
  Result := DaysPerMonth(Year, Month);
end;

procedure TCalendar2K.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TGridDrawState);
var
  TheText: string;
begin
...
function TCalendar2K.GetCellText(ACol, ARow: Integer): string;
var
  DayNum: Integer;
begin
...
function TCalendar2K.SelectCell(ACol, ARow: Longint): Boolean;
begin
...
procedure TCalendar2K.SetCalendarDate(Value: TDateTime);
begin
  FDate := Value;
  UpdateCalendar;
  Change;
...
function TCalendar2K.StoreCalendarDate: Boolean;
begin
  Result := not FUseCurrentDate;
end;

function TCalendar2K.GetDateElement(Index: Integer): Integer;
var
...
procedure TCalendar2K.SetDateElement(Index: Integer; Value: Integer);
var
  AYear, AMonth, ADay: Word;
begin
  if Value > 0 then
...
procedure TCalendar2K.SetStartOfWeek(Value: TDayOfWeek);
begin
...
procedure TCalendar2K.SetUseCurrentDate(Value: Boolean);
begin
...
procedure TCalendar2K.ChangeMonth(Delta: Integer);
var
  AYear, AMonth, ADay: Word;
  NewDate: TDateTime;
  CurDay: Integer;
...
procedure TCalendar2K.PrevMonth;
begin
  ChangeMonth(-1);
end;

procedure TCalendar2K.NextMonth;
begin
  ChangeMonth(1);
end;

procedure TCalendar2K.NextYear;
begin
  if IsLeapYear(Year) and (Month = 2) and (Day = 29) then Day := 28;
  Year := Year + 1;
end;

procedure TCalendar2K.PrevYear;
begin
  if IsLeapYear(Year) and (Month = 2) and (Day = 29) then Day := 28;
  Year := Year - 1;
end;

procedure TCalendar2K.UpdateCalendar;
var
  AYear, AMonth, ADay: Word;
  FirstDate: TDateTime;
begin
...
procedure TCalendar2K.WMSize(var Message: TWMSize);
var
  GridLines: Integer;
begin
...
procedure Register;
begin
  RegisterComponents('Neue', [TCalendar2K]);
end;
