...
function getVirtMemProz(): String;
begin
     result := intToStr(Round(strToInt(getAvailPageFile)*100 / strToInt(getTotalPageFile)));
end;

function getPhysMemProz(): String;
begin
     result := intToStr(Round(strToInt(getAvailPhysMemory)*100 / strToInt(getTotalPhysMemory)));
end;
//****************************************************************************
//*************        Programmfunktionen
//****************************************************************************
...
begin
     ...
     If lowercase(tag) = 'freevirtualmemproz' then result := getVirtMemProz;
     If lowercase(tag) = 'freephysmemproz' then result := getPhysMemProz;
...
// Jedes Zeichen wird nach einem Tag abgesucht
    For i := 1 to (length(oldhtml)) do begin
        If (oldhtml[i] = '<') and (oldhtml[i + 1] = '%') then begin
           tagStart := i; // Damit nur der Tag übrigbleibt
        end else
...
procedure TCreateHTML.ServiceStop(Sender: TService; var Stopped: Boolean);
begin
//     CreateHTML.DoShutdown;
     LogMessage('Der Dienst createHTMLinfo wurde beendet!', EVENTLOG_INFORMATION_TYPE, 0, 0);
